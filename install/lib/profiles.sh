...
# Profiles:
#   minimal  → shell only (no dots, no extras)
#   core     → dots + shell (default, recommended)
#   full     → core + bundled fonts/bibata
#   ultra    → full + sysupdate + build-from-source for everything + microtex

# Components (canonical):
#   dots, shell, hyprland, fish, fontconfig, miscconf, bundled, deps, sysupdate, build, backup

declare -A PROFILE_DESC=(
    [minimal]="Shell only — minimal Quickshell config, no base dotfiles"
    [core]="Core — dots + shell (default)"
...
# Sets globals: DO_DOTS, DO_SHELL, DO_BUNDLED, DO_BUILD, DO_SYSUPDATE, DO_DEPS, DO_BACKUP
...
horizons_profile_resolve() {
    ...
    export DO_DOTS DO_SHELL DO_BUNDLED DO_BUILD DO_SYSUPDATE DO_DEPS DO_BACKUP
}
...
# Usage: horizons_components_apply "dots,shell"
...
horizons_components_apply() {
    ...
    for tok in "${parts[@]}"; do
        ...
        case "$tok" in
            ...
            all)  DO_DOTS=true; DO_SHELL=true; DO_BUNDLED=true; DO_BUILD=true; DO_DEPS=true ;;
            none) DO_DOTS=false; DO_SHELL=false; DO_BUNDLED=false; DO_BUILD=false; DO_DEPS=false ;;
