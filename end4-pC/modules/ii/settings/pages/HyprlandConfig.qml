...
ContentPage {
    ...
    Component.onCompleted: {
        ...
        HyprlandConfig.setMany({
            ...
            "input:scroll_lock":                        h.input.scrollLock ? 1 : 0,
            "input:scroll_point_scroll":                h.input.scrollPointScroll ? 1 : 0,
            ...
            "input:touchpad:dragfinger_distance":       h.input.touchpad.dragfingerDistance,
            "input:touchpad:emulate_scroll":            h.input.touchpad.emulateScroll ? 1 : 0,
            "input:touchpad:emulation_scroll_factor":   h.input.touchpad.emulationScrollFactor,
        })
    }
        ...
        ContentSection {
            ...
            ContentSubsection {
                ...
                ContentSubsection {
                    ...
                    GroupedList {
                        ...
                        ConfigSpinBox {
                            icon: "drag_click"
                            text: Translation.tr("Drag Finger Distance")
                            value: Config.options.hyprland.input.touchpad.dragfingerDistance
                            from: 0; to: 10; stepSize: 1
                            onValueChanged: {
                                if (value === Config.options.hyprland.input.touchpad.dragfingerDistance) return
                                Config.options.hyprland.input.touchpad.dragfingerDistance = value
                                HyprlandConfig.set("input:touchpad:dragfinger_distance", value)
                            }
                        }
                        ConfigSwitch {
                            buttonIcon: "swap_vert"
                            text: Translation.tr("Emulate Scroll")
                            checked: Config.options.hyprland.input.touchpad.emulateScroll
                            onCheckedChanged: {
                                if (checked === Config.options.hyprland.input.touchpad.emulateScroll) return
                                Config.options.hyprland.input.touchpad.emulateScroll = checked
                                HyprlandConfig.set("input:touchpad:emulate_scroll", checked ? 1 : 0)
                            }
                        }
                        ConfigSpinBox {
                            icon: "swap_vert"
                            text: Translation.tr("Emulation Scroll Factor")
                            value: Math.round(Config.options.hyprland.input.touchpad.emulationScrollFactor * 10)
                            from: 1; to: 30; stepSize: 1
                            enabled: Config.options.hyprland.input.touchpad.emulateScroll
                            onValueChanged: {
                                const v = value / 10.0
                                if (v === Config.options.hyprland.input.touchpad.emulationScrollFactor) return
                                Config.options.hyprland.input.touchpad.emulationScrollFactor = v
                                HyprlandConfig.set("input:touchpad:emulation_scroll_factor", v)
                            }
                        }
                    }
                }
            ...
            ContentSubsection {
                ...
                GroupedList {
                    ...
                    ConfigSwitch {
                        buttonIcon: "lock"
                        text: Translation.tr("Scroll Lock")
                        checked: Config.options.hyprland.input.scrollLock
                        onCheckedChanged: {
                            if (checked === Config.options.hyprland.input.scrollLock) return
                            Config.options.hyprland.input.scrollLock = checked
                            HyprlandConfig.set("input:scroll_lock", checked ? 1 : 0)
                        }
                    }
                    // Scroll Point Scroll
                    ConfigSwitch {
                        buttonIcon: "touch_app"
                        text: Translation.tr("Scroll Point Scroll")
                        checked: Config.options.hyprland.input.scrollPointScroll
                        onCheckedChanged: {
                            if (checked === Config.options.hyprland.input.scrollPointScroll) return
                            Config.options.hyprland.input.scrollPointScroll = checked
                            HyprlandConfig.set("input:scroll_point_scroll", checked ? 1 : 0)
                        }
                    }
                    // Left Handed
                    ConfigSwitch {
        ...
        ContentSection {
            ...
            ContentSubsection {
                ...
                GroupedList {
                    // Blur Variant
                    ConfigSelectionArray {
                        text: Translation.tr("Blur Variant")
                        icon: "blur_on"
                        ...
                        options: [
                            { displayName: Translation.tr("Default"), icon: "blur_on", value: 0 },
                            { displayName: "Kawase", icon: "blur_circular", value: 1 },
                            { displayName: Translation.tr("Dual Kawase"), icon: "blur_long_on", value: 2 }
                        ]
                    }
                    // Noise
                    ConfigSlider {
