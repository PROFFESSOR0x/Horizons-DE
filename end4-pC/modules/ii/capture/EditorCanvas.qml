...
Item {
    ...
    property real defaultAnnotationDuration: 3.0
    ...
    // ---------------------------------------------------------------------------
    function saveImage(originalPath) {
        if (!originalPath) return
        let savePath = originalPath.indexOf(".") !== -1
            ? originalPath.replace(/\.[^.]+$/, "-edited.png")
            : originalPath + "-edited.png"

        let nw = root.contentNativeWidth
        let nh = root.contentNativeHeight

        // If there are no annotations just grab the container as-is (fast path).
        if (root.drawHistory.length === 0) {
            canvasContainer.grabToImage(function(result) {
                // grabToImage returns display-sized PNG; upscale via ImageMagick
                // to native resolution so the output matches the original.
                let tmp = "/tmp/qs-save-nodraw.png"
                ...
                console.log("[EditorCanvas] Image saved (no annotations):", savePath)
            })
            return
        }
        ...
        renderAnnotationsOffscreen(nw, nh, null, function(annPath) {
            Quickshell.execDetached([
                "convert",
                originalPath,           // original background (native size)
                annPath,                // annotation layer (native size, transparent bg)
                "-composite",
                savePath
            ])
            console.log("[EditorCanvas] Image saved:", savePath)
        })
    }
    ...
    function copyToClipboard() {
        ...
        if (root.drawHistory.length === 0 || root.isVideo) {
            ...
            canvasContainer.grabToImage(function(result) {
                let tempPath = "/tmp/capture-editor-copy.png"
                result.saveToFile(tempPath)
                Quickshell.execDetached(["bash", "-c", "cat '" + tempPath + "' | wl-copy"])
            })
            return
        }

        let annTmp = "/tmp/qs-copy-ann.png"
        let outTmp = "/tmp/capture-editor-copy.png"
        renderAnnotationsOffscreen(nw, nh, null, function(annPath) {
            Quickshell.execDetached([
                "bash", "-c",
                "convert '" + root.loadedImagePath + "' '" + annPath + "' -composite '" + outTmp + "' && cat '" + outTmp + "' | wl-copy"
            ])
        })
    }
