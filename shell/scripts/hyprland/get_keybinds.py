...
class KeyBinding(dict):
    def __init__(self, mods, key, dispatcher, params, comment, key_str="", rest=""):
        self["mods"]       = mods
        self["key"]        = key
...
def parse_lua_bind(line: str, override_comment: str = "") -> Optional[KeyBinding]:
    ...
    if disp_match:
        ...
        dispatcher = "function"
        params     = ""
        # Keep full rest for function to allow reconstruction
    else:
    ...
    if not comment:
        # Fallback: show dispatcher or key as comment so bind is not hidden
        comment = dispatcher if dispatcher and dispatcher != "function" else key_str

    mods, key = parse_key_string(key_str)
    return KeyBinding(mods, key, dispatcher, params, comment, key_str, rest)
    ...
    while reading_line < len(content_lines):
        ...
        if comment_bind_match:
            ...
            if rest.startswith("hl.bind"):
                ...
                kb = KeyBinding([], key_hint, "comment", "", comment_part, key_hint, "")
                current_content["keybinds"].append(kb)
            reading_line += 1
            continue
