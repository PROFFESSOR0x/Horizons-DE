...
Singleton {
    ...
    function switchLayoutNext() {
        if (WM.compositor !== "hyprland") return
        if (keyboardName.length > 0) Quickshell.execDetached(["hyprctl", "switchxkblayout", keyboardName, "next"])
        else Quickshell.execDetached(["hyprctl", "switchxkblayout", "next"])
    }
    function switchLayoutPrev() {
        if (WM.compositor !== "hyprland") return
        if (keyboardName.length > 0) Quickshell.execDetached(["hyprctl", "switchxkblayout", keyboardName, "prev"])
        else Quickshell.execDetached(["hyprctl", "switchxkblayout", "prev"])
    }
        ...
        stdout: StdioCollector {
            ...
            onStreamFinished: {
                try {
                    ...
                    const keyboards = Array.isArray(parsedOutput?.keyboards) ? parsedOutput.keyboards : []
                    ...
                    root.keyboardName = hyprlandKeyboard.name ?? ""
                } catch (error) {
                    console.log("[HyprlandXkb] Failed to parse devices: " + error)
                }
            }
        }
    }
