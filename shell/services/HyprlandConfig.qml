...
Singleton {
    ...
    readonly property string customBindsPath: FileUtils.trimFileProtocol(`${Directories.config}/hypr/custom/keybinds.lua`)
    readonly property string customRulesPath: FileUtils.trimFileProtocol(`${Directories.config}/hypr/custom/rules.lua`)
    ...
    function setCustomAnims(curvesJson: string, animsJson: string) {
        Quickshell.execDetached([
            "python3", root.configuratorScriptPath,
            "--anim-curves-json", curvesJson,
            "--anim-anims-json", animsJson,
            "--anim-file", root.animOverridesPath
        ])
    }
