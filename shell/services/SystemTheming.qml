...
Singleton {
    ...
    // Default terminal: the shell's own actions read
    // Config.options.apps.terminal live, while Hyprland shortcuts
    // (Super+Return, Super+T, Ctrl+Alt+T) and GNOME consumers are updated by
    // the script (custom/variables.lua + custom/variables.conf + reload).
    function applyDefaultTerminal(command) {
        const cmd = (command ?? "").trim()
        if (cmd.length === 0) return
        Quickshell.execDetached(["bash", `${root.scriptDir}/set-default-terminal.sh`, cmd])
    }
