...
Singleton {
    ...
    // { "<monitor name>": true } while that monitor's active workspace has a
    // non-floating (or fullscreen) window on it, i.e. the desktop behind it is
    // not actually visible. Read it as a property, not through a function, so
    // bindings re-evaluate when windows move - see HyprlandBackend.
    readonly property var obscuredMonitors: backend?.obscuredMonitors ?? ({})
    ...
    function switchWorkspacesOnMonitors(entries, focusMonitor, windowToFocus) {
        if (backend?.switchWorkspacesOnMonitors) backend.switchWorkspacesOnMonitors(entries, focusMonitor, windowToFocus)
    ...
    function monitorObscured(monitorName) { return !!root.obscuredMonitors[monitorName] }
