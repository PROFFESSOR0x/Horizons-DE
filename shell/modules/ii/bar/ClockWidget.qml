...
BarWidgetSwitcher {
    ...
    property bool showDate: Config?.options?.time?.showDate ?? true
    ...
    rowDefault: Component {
        RowLayout {
            spacing: 4
            StyledText {
                visible: (Config?.options?.time?.showDate ?? true)
            ...
            StyledText {
                ...
                // Pop on time change
                scale: 1
                Behavior on scale { animation: Appearance.animation.elementMoveSmall.numberAnimation.createObject(timeTextRow) }
                onTextChanged: {
                    timePop.restart()
                }
                SequentialAnimation {
                    id: timePop
                    NumberAnimation { target: timeTextRow; property: "scale"; from: 0.90; to: 1.04; duration: 140; easing.type: Easing.OutCubic }
                    NumberAnimation { target: timeTextRow; property: "scale"; from: 1.04; to: 1; duration: 220; easing.type: Easing.OutBack }
                }
                // Subtle colon pulse via opacity
                SequentialAnimation on opacity {
                    loops: Animation.Infinite
                    running: (Config?.options?.time?.secondPrecision ?? false)
                    NumberAnimation { from: 1; to: 0.55; duration: 600; easing.type: Easing.InOutQuad }
                    NumberAnimation { from: 0.55; to: 1; duration: 600; easing.type: Easing.InOutQuad }
                }
            }
        }
    }
