...
PopupWindow {
    ...
    MouseArea {
        ...
        Rectangle {
            ...
            scale: 0.92
            // Slide from bar edge
            y: Config.options.bar.bottom ? -6 : 6
            Component.onCompleted: { opacity = 1; scale = 1; y = 0 }
            ...
            Behavior on scale {
                animation: Appearance.animation.clickBounce.numberAnimation.createObject(popupBackground)
            }
            Behavior on y {
                animation: Appearance.animation.elementMoveSmall.numberAnimation.createObject(popupBackground)
            }
