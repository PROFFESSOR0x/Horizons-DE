...
Item {
    ...
    Rectangle {
        ...
        Behavior on color { animation: Appearance.animation.elementMoveFast.colorAnimation.createObject(barBackground) }
        Behavior on radius { animation: Appearance.animation.elementResize.numberAnimation.createObject(barBackground) }
        Behavior on border.width { animation: Appearance.animation.elementMoveFast.numberAnimation.createObject(barBackground) }
        Behavior on anchors.margins { animation: Appearance.animation.elementMove.numberAnimation.createObject(barBackground) }
        // Subtle elevation morph on hover via parent mustShow is handled in Bar.qml, here ensure smooth transparent→color
        opacity: Config.options.bar.showBackground ? 1 : 0
    ...
    Rectangle {
        ...
        scale: centerOnly ? 1 : 0.92
        opacity: centerOnly ? 1 : 0
        ...
        Behavior on width { animation: Appearance.animation.elementResize.numberAnimation.createObject(centerPill) }
        Behavior on height { animation: Appearance.animation.elementResize.numberAnimation.createObject(centerPill) }
        Behavior on scale { animation: Appearance.animation.elementMoveSmall.numberAnimation.createObject(centerPill) }
        Behavior on opacity { NumberAnimation { duration: 220; easing.type: Easing.OutCubic } }
        Behavior on color { animation: Appearance.animation.elementMoveFast.colorAnimation.createObject(centerPill) }
    }
