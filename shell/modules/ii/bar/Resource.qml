...
Item {
    ...
    scale: warning ? 1.04 : 1
    // Springy fill when percentage jumps
    property real animPerc: percentage
    Behavior on animPerc { animation: Appearance.animation.elementMove.numberAnimation.createObject(root) }
    onPercentageChanged: {
        animPerc = percentage
        if (Math.abs(percentage - animPerc) > 0.08) bounceAnim.restart()
    }
    SequentialAnimation {
        id: bounceAnim
        NumberAnimation { target: root; property: "scale"; from: 0.92; to: 1.06; duration: 160; easing.type: Easing.OutCubic }
        NumberAnimation { target: root; property: "scale"; from: 1.06; to: 1; duration: 280; easing.type: Easing.OutBack }
    }
    Behavior on scale { animation: Appearance.animation.elementMoveSmall.numberAnimation.createObject(root) }
