...
Item { // Window
    ...
    property real widthRatio: {
        ...
        if (!monitorWidth || !widgetMonitor.scale) return 1;
    ...
    property real heightRatio: {
        ...
        if (!monitorHeight || !widgetMonitor.scale) return 1;
        return (widgetHeight * monitorData.scale) / (monitorHeight * widgetMonitor.scale);
    }
    property real initX: {
        const windowX = windowData && windowData.at ? windowData.at[0] : 0;
        const reservedLeft = monitorData && monitorData.reserved ? monitorData.reserved[0] : 0;
        return Math.max((windowX - (monitorData?.x ?? 0) - reservedLeft) * widthRatio * root.scale, 0) + xOffset;
    }

    property real initY: {
        const windowY = windowData && windowData.at ? windowData.at[1] : 0;
        const reservedTop = monitorData && monitorData.reserved ? monitorData.reserved[1] : 0;
        return Math.max((windowY - (monitorData?.y ?? 0) - reservedTop) * heightRatio * root.scale, 0) + yOffset;
    }
    property real xOffset: 0
    property real yOffset: 0
    property var widgetMonitor
    property int widgetMonitorId: widgetMonitor ? widgetMonitor.id : -1

    property real targetWindowWidth: (windowData && windowData.size ? windowData.size[0] : 0) * scale * widthRatio
    property real targetWindowHeight: (windowData && windowData.size ? windowData.size[1] : 0) * scale * heightRatio
